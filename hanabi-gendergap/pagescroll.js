class PageScroll{
    ...
    #setup() {
        ...
        this.$sections.forEach(elem => elem.style.transition = `transform ${+this.opts.animDuration/1000 + 's' || '30s'}`)
        this.$sections.forEach(elem => elem.style.transitionTimingFunction = this.opts.easing ? this.opts.easing : 'cubic-bezier(0.5)')
    ...
    moveup() {
        if (this.level != 0 && !this.isAnimating) {
            ...
            //this.$sections.forEach(elem => elem.style.opacity = `1` )
